...
from edxapp_acceptance.pages.common.utils import click_and_wait_for_window
...
def press_the_notification_button(page, name):
    ...
    page.browser.execute_script(f"$('{btn_css}').focus().click()")
    page.wait_for_ajax()
...
def add_advanced_component(page, menu_index, name):
    ...
    component_css = f'button[data-category={name}]'
    page.wait_for_element_visibility(component_css, f'Advanced component {name} is visible')
...
def add_component(page, item_type, specific_type, is_advanced_problem=False):
    ...
    """
    btn = page.q(css=f'.add-xblock-component .add-xblock-component-button[data-type={item_type}]')
    multiple_templates = btn.filter(lambda el: 'multiple-templates' in el.get_attribute('class')).present
    btn.click()
    if multiple_templates:
        sub_template_menu_div_selector = f'.new-component-{item_type}'
        ...
        all_options = page.q(css=f'.new-component-{item_type} ul.new-component-template li button span')
...
def add_html_component(page, menu_index, boilerplate=None):
    ...
    if boilerplate:
        component_css += f'[data-boilerplate={boilerplate}]'
    else:
        component_css += ':not([data-boilerplate])'

    page.wait_for_element_visibility(component_css, f'HTML component {boilerplate} is visible')
        ...
        for xblock in xblocks:
            ...
            if xblock.name == parent:
                ...
                test_class.assertEqual(
                    ...
                    "Number of children incorrect for group {}. Expected {} but got {}.".format(
                        parent, expected_length, len(children)
                    ))
