...
from edxapp_acceptance.pages.common.utils import click_css, confirm_prompt
...
from edxapp_acceptance.pages.studio.utils import set_input_value, set_input_value_and_save
from edxapp_acceptance.pages.common.utils import disable_animations, enable_animations, select_option_by_text
...
class CourseOutlineItem:
    ...
    def has_status_message(self):
        ...
        """
        return self.q(
            css=self._bounded_selector(self.STATUS_MESSAGE_SELECTOR)
        ).first.visible  # pylint: disable=no-member
    ...
    def is_staff_only(self):
        ...
        return "is-staff-only" in self.q(
            css=self._bounded_selector('')
        )[0].get_attribute("class")  # pylint: disable=no-member
    ...
    def toggle_unit_access(self, partition_name, group_ids):
        ...
        if group_ids:
            ...
            for group_id in group_ids:
                checkbox = self.q(css=f'#content-group-{group_id}')
                checkbox.click()
            modal.save()
    ...
    def edit(self):
        ...
        """
        self.q(
            css=self._bounded_selector(self.CONFIGURATION_BUTTON_SELECTOR)
        ).first.click()  # pylint: disable=no-member
...
class CourseOutlineContainer(CourseOutlineItem):
    ...
    def expand_subsection(self):
        ...
        EmptyPromise(
            ...
            f"Check that the container {self.locator} has been toggled"
        ).fulfill()
...
class CourseOutlinePage(CoursePage, CourseOutlineContainer):
    ...
    def click_section_name(self, parent_css=''):
        ...
        """
        self.q(css=f'{parent_css} .section-name').first.click()
    ...
    def get_section_name(self, parent_css='', page_refresh=False):
        ...
        return self.q(css=f'{parent_css} .section-name').text

    def section_name_edit_form_present(self, parent_css=''):
        """
        Check that section name edit form present
        """
        return self.q(css=f'{parent_css} .section-name input').present
    ...
    def change_section_name(self, new_name, parent_css=''):
        ...
        self.q(css=f'{parent_css} .section-name input').first.fill(new_name)
        self.q(css=f'{parent_css} .section-name .save-button').first.click()
        self.wait_for_ajax()
...
class CourseOutlineModal:
    ...
    def set_date(self, property_name, input_selector, date):
        ...
        EmptyPromise(
            lambda: getattr(self, property_name) == f'{month}/{day}/{year}',
            f"{property_name} is updated in modal."
        ).fulfill()
