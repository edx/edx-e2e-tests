...
class VideoPage(PageObject):
    ...
    def is_video_rendered(self, mode):
        ...
        return Promise(_is_element_present, f'Video Rendering Failed in {mode} mode.').fulfill()
    ...
    def _closed_captions_visibility(self, closed_captions_new_state):
        ...
        EmptyPromise(lambda: self.is_closed_captions_visible() == closed_captions_new_state,
                     f"Closed captions are {state}").fulfill()
    ...
    def speed(self, speed):
        ...
        speed_selector = self.get_element_selector(f'li[data-speed="{speed}"] .control')
    ...
    def select_language(self, code):
        ...
        language_selector = VIDEO_MENUS["language"] + f' li[data-lang-code="{code}"]'
    ...
    def state(self):
        ...
        logging.debug(f"Current state of '{state_selector}' element is '{current_state}'")
    ...
    def _wait_for(self, check_func, desc, result=False, timeout=200, try_interval=0.2):
        ...
        Arguments:
            check_func (callable): Function that accepts no arguments and returns a boolean indicating
    ...
    def seek(self, seek_value):
        ...
        js_code = "$('{seek_selector}').data('video-player-state')." \
                  "videoPlayer.onSlideSeek({{time: {seek_time}}})".format(
