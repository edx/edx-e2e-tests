...
from edxapp_acceptance.pages.common.utils import get_options, get_selected_option_text, select_option_by_text
...
class DiscussionManagementSection(PageObject):
    ...
    def _bounded_selector(self, selector):
        ...
        """
        return f'.discussions-management {selector}'
...
class MembershipPageBetaTesterSection(PageObject):
    ...
    def fill_batch_beta_tester_addition_text_box(self, username):
        ...
        """
        username_selector = f"{self.batch_beta_tester_selector} textarea"
        enrollment_button = f"{self.batch_beta_tester_selector} .enrollment-button[data-action='add']"
    ...
    def get_notification_text(self):
        ...
        Check notification div is visible and have message.
        """
        notification_selector = f'{self.batch_beta_tester_selector} .request-response'
        self.wait_for_element_visibility(notification_selector, 'Notification div is visible')
        notification_header_text = self.q(css=f"{notification_selector} h3").text
        notification_username = self.q(css=f"{notification_selector} li").text
...
class StudentAdminPage(PageObject):
    ...
    def _input_with_name(self, input_name):
        ...
        """
        return self.q(css=f'{self.CONTAINER} input[name={input_name}]')
    ...
    def wait_for_task_history_table(self):
        ...
        def check_func():
            ...
            Promise Check Function
            """
            query = self.q(css=f"{self.CONTAINER} .{self.TASK_HISTORY_TABLE_NAME}")
...
class GradeBookPage(StudentAdminPage):
    ...
    def get_value_in_the_grade_book(self, title, index):
        ...
        return self.q(css=f'.grade-table td[title^="{title}"]').text[index]
