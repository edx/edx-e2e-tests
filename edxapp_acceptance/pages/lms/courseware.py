...
from edxapp_acceptance.pages.lms.completion import CompletionOnViewMixin
...
class CoursewarePage(CoursePage, CompletionOnViewMixin):
    ...
    def xblock_component_html_content(self, index=0):
        ...
        element = self.q(css=f'{self.xblock_component_selector} .edx-notes-wrapper-content')
        if element.first:
    ...
    def verify_tooltips_displayed(self):
        ...
        for index, tab in enumerate(self.q(css='#sequence-list > li')):
            ...
            self.wait_for_element_visibility(
                f'#tab_{index} > .sequence-tooltip',
                f'Tab {index} should appear'
            )
    ...
    def active_usage_id(self):
        ...
        def get_active(el): return 'active' in el.get_attribute('class')

        def attribute_value(el): return el.get_attribute('data-id')
...
class CoursewareSequentialTabPage(CoursePage):
    ...
    def __init__(self, browser, course_id, chapter, subsection, position):
        super().__init__(browser, course_id)
        self.url_path = f"courseware/{chapter}/{subsection}/{position}"
...
class CourseNavPage(PageObject):
    ...
    def sections(self):
        ...
        for sec_index, sec_title in enumerate(section_titles):
            ...
            if len(section_titles) < 1:
                self.warning(f"Could not find subsections for '{sec_title}'")
            else:
    ...
    def go_to_section(self, section_title, subsection_title):
        ...
        try:
            ...
            msg = f"Could not find subsection '{subsection_title}' in section '{section_title}'"
            self.warning(msg)
            return
    ...
    def _on_section_promise(self, section_title, subsection_title):
        ...
        """
        desc = f"currently at section '{section_title}' and subsection '{subsection_title}'"
        return EmptyPromise(
